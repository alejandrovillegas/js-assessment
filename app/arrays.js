arraysAnswers = {
  ...
  sum: function sum(arr) {
    return arr.reduce((previusiItem, ActualItem) => previusiItem + ActualItem, 0);
  },
  ...
  duplicates: function duplicates(arr) {
    ...
    for (let i = 0; i < newArr.length - 1; i++) {
      if (newArr[i + 1] === newArr[i] && results[results.length - 1] !== newArr[i]) {
  ...
  square: function square(arr) {
    return arr.map(arrItem => arrItem * arrItem);
  },
